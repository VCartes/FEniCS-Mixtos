...
u1_ex = sp.sin(sp.pi*x) * sp.cos(sp.pi*y) * sp.sin(sp.pi*z)
u2_ex = sp.cos(sp.pi*x) * sp.sin(sp.pi*y) * sp.sin(sp.pi*z)
u3_ex = 2 * sp.cos(sp.pi*x) * sp.cos(sp.pi*y) * sp.cos(sp.pi*z)

p_ex = sp.cos(2*sp.pi*x) * sp.exp(y) * sp.sin(z)


chi11_ex = u1_ex.diff(x, 1)
chi12_ex = u1_ex.diff(y, 1)
chi13_ex = u1_ex.diff(z, 1)
chi21_ex = u2_ex.diff(x, 1)
chi22_ex = u2_ex.diff(y, 1)
chi23_ex = u2_ex.diff(z, 1)
chi31_ex = u3_ex.diff(x, 1)
...
sigma13_ex = mu_val * chi13_ex - 1/2 * u1_ex * u3_ex
...
u_ex        = Expression((sp.printing.ccode(u1_ex),
                          sp.printing.ccode(u2_ex),
                          sp.printing.ccode(u3_ex)), degree = 5)
p_ex        = Expression(sp.printing.ccode(p_ex), degree = 5)
chi_ex      = Expression([
    [sp.printing.ccode(chi11_ex), sp.printing.ccode(chi12_ex), sp.printing.ccode(chi13_ex)], 
    [sp.printing.ccode(chi21_ex), sp.printing.ccode(chi22_ex), sp.printing.ccode(chi23_ex)],
    [sp.printing.ccode(chi31_ex), sp.printing.ccode(chi32_ex), sp.printing.ccode(chi33_ex)]], 
                         degree = 5)
sigma_ex    = Expression([
    [sp.printing.ccode(sigma11_ex), sp.printing.ccode(sigma12_ex), sp.printing.ccode(sigma13_ex)],
    [sp.printing.ccode(sigma21_ex), sp.printing.ccode(sigma22_ex), sp.printing.ccode(sigma23_ex)],
    [sp.printing.ccode(sigma31_ex), sp.printing.ccode(sigma32_ex), sp.printing.ccode(sigma33_ex)]]
                         , degree = 5)

f = Expression((sp.printing.ccode(f1), 
                sp.printing.ccode(f2),
                sp.printing.ccode(f3)), degree = 5)
...
def build_trace_free_tensor(components):
    tensor = as_tensor(
            [[components[0], components[1], components[2]], 
             ...
             [components[6], components[7], -components[0] - components[4]]]
            )
...
def Hdiv43_tensor_norm(f):
    ...
    divf = div(f)
    norm += np.pow(assemble(sum(divf[i]**(4/3) for i in range(3)) * dx(metadata={"quadrature_degree": 5})), 3/4)
...
def solve_variational_problem(mesh):
    ...
    # For L_2^{\tr}(\Omega)
    Hchi = MixedElement([DG for _ in range(8)])

    # For H_0(div_{4/3}, \Omega)
    RT = FiniteElement("RT", mesh.ufl_cell(), order+1)
    Hsigma = MixedElement([RT, RT, RT])
    ...
    tensor_V = TensorFunctionSpace(mesh, "DG", order, shape = (3, 3))
...
if __name__ == "__main__":
    ...
    u_file      = File("Data_Paraview_3D/u.pvd")
    chi_file    = File("Data_Paraview_3D/chi.pvd")
    sigma_file  = File("Data_Paraview_3D/sigma.pvd")
    p_file      = File("Data_Paraview_3D/p.pvd")
